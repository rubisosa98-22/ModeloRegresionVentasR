...
for i, m in enumerate(macros_present):
    ...
    # Mostrar como porcentaje con 1 decimal, pero guardar internamente en decimales
    val_pct = (float(v) * 100) if pd.notna(v) else 0.0
    input_pct = edit_cols[i].number_input(f"{m} ({fore_year}) [%]", value=round(val_pct,1), step=0.1, format="%.1f")
    fore_inputs[m] = input_pct / 100.0
